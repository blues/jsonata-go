...
func TestTransformFunctions(t *testing.T) {
	...
	t.Run("SimpleTransform", func(t *testing.T) {
		expr, err := jsonata.Compile("Account.Order.Product.{\"name\": `Product Name`, \"id\": ProductID}")
		if err != nil {
	...
	t.Run("TransformWithMap", func(t *testing.T) {
		expr, err := jsonata.Compile("Account.Order.Product ~> $map(function($p) { { \"name\": $p.`Product Name`, \"price\": $p.Price } })")
		if err != nil {
			t.Fatalf("Failed to compile expression: %v", err)
		}
	...
	t.Run("GroupByTransform", func(t *testing.T) {
		expr, err := jsonata.Compile(`Account.Order.{
			"id": OrderID,
			"products": Product.{
				"name": ` + "`Product Name`" + `,
				"price": Price
	...
	t.Run("TransformWithFilter", func(t *testing.T) {
		expr, err := jsonata.Compile(`Account.Order.Product[Price > 30].{
			"name": ` + "`Product Name`" + `,
			"price": Price
		}`)
		if err != nil {
